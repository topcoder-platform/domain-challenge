...
class LegacyMapper {
  ...
  private mapScorecard(subTrack: string, phaseTypeId: number| undefined): string | undefined {
    ...
    // F2F
    if (subTrack === V4_SUBTRACKS.FIRST_2_FINISH && phaseTypeId === PhaseTypeIds.IterativeReview) scorecard = isNonProd ? 30002160 : 30002160; // missing dev scorecard
    if (subTrack === V4_SUBTRACKS.DESIGN_FIRST_2_FINISH && phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30001610 : 30001101; // missing dev scorecard

    // QA
    if (subTrack === V4_SUBTRACKS.BUG_HUNT) {
      if (phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30001610 : 30001220; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.SpecificationReview) scorecard = isNonProd ? 30001610 : 30001120; // missing dev scorecard
    }

    // DS
    if (subTrack === V4_SUBTRACKS.DEVELOP_MARATHON_MATCH && phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30001610 : 30002133; // missing dev scorecard
    if (subTrack === V4_SUBTRACKS.MARATHON_MATCH && phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30001610 : 30002133; // missing dev scorecard

    // DESIGN
    if (subTrack === V4_SUBTRACKS.WEB_DESIGNS) {
      if (phaseTypeId === PhaseTypeIds.SpecificationReview) scorecard = isNonProd ? 30001610 : 30001040; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.CheckpointScreening) scorecard = isNonProd ? 30001610 : 30001364; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.CheckpointReview) scorecard = isNonProd ? 30001610 : 30001004; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.Screening) scorecard = isNonProd ? 30001610 : 30001363; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30001610 : 30001031; // missing dev scorecard
      if (phaseTypeId === PhaseTypeIds.Approval) scorecard = isNonProd ? 30001610 : 30000720; // missing dev scorecard
    }

    if (subTrack === V4_SUBTRACKS.CODE && phaseTypeId === PhaseTypeIds.Review) scorecard = isNonProd ? 30002133 : 30002133; // missing dev scorecard
